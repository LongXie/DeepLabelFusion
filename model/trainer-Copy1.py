...
class UNet3DLabelFusionTrainer:
    ...
    def __init__(self, model, optimizer, lr_scheduler, loss_criterion,
                 ...
                 validate_iters=None, num_iterations=1, num_epoch=0,
                 eval_score_higher_is_better=True, best_eval_score=None,
                 logger=None, tensorboard_formatter=None, skip_train_validation=False, use_weighted_loss = 0, 
                 step_after_iters = 1, use_designed_loss = 0, 
                 deep_supervision = False, deep_supervision_weights = None):
        if logger is None:
            self.logger = get_logger('UNet3DLabelFusionTrainer', level=logging.DEBUG)
        else:
            self.logger = logger
    ...
    def train(self, train_loader):
        ...
        for i, t in enumerate(train_loader):
            ...
            output, loss, seg_targets, weightmap, output_init = self._forward_pass(input, target, coordmaps, weight)
    ...
    def validate(self, val_loader):
        ...
        with torch.no_grad():
            for i, t in enumerate(val_loader):
                ...
                output_val, loss_val, seg_targets_val, _, _ = self._forward_pass(input_val, target_val, coordmaps_val, weight_val)
    ...
    def _forward_pass(self, input, target, coordmaps, weight=None):
        ...
        #print(weightmaps.size())
        #print(output.size())
        #print(weightmap_output.size())
        
        # multiply the label maps
        #N_label = 13
        #probmaps = torch.zeros(nbt, (npt-1), N_label, nx, ny, nz)
        #for i in range(nbt):
        #    for j in range(npt-1):
        #        idx = i * (npt-1) + j 
        #        for l in range(N_label):
        #            labelmap = seg_atlases[i, j, ...] == l
        #            labelmap = labelmap.type(torch.cuda.FloatTensor)
        #            probmaps[i, j, l, ...] = weightmap_output[idx, 0, ...] * labelmap
        #output = probmaps.mean(1)
        #output = self._move_to_device(output)
        #print(probmaps.size())
        #print(output.size())

        #probability = F.softmax(output, dim = 1)
        #_, preds_tensor = torch.max(probability, 1)
        #preds_tensor = preds_tensor.type(torch.cuda.FloatTensor)
        ...
        return output, loss, seg_targets, weightmaps_out, output_init
...
def get_trainer(config):
    ...
    if name == "UNet3DTrainer" or name == 'UNet3DMultimodalTrainer' or name == 'UNet3DTrainerGetModel' or name == "UNet3DLabelFusionTrainer":
